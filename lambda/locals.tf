locals {
  ...
  transform_engine_v2_retry_queue_name      = "${var.project}-transform-engine-v2-retry-${local.environment}"
  transform_engine_v2_retry_queue           = "arn:aws:sqs:${var.region}:${data.aws_caller_identity.current.account_id}:${local.transform_engine_v2_retry_queue_name}"
}
