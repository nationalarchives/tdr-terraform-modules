...
resource "aws_iam_policy" "transform_engine_notifications_lambda_policy" {
  count  = local.transform_engine_count
  policy = templatefile("${path.module}/templates/notifications_transform_engine_lambda.json.tpl", { transform_engine_output_queue_arn = data.aws_ssm_parameter.transform_engine_output_sqs_arn[0].value, transform_engine_retry_queue_arn = local.transform_engine_retry_queue, transform_engine_v2_retry_queue_arn = local.transform_engine_v2_retry_queue,transform_engine_in_topic_arn = data.aws_ssm_parameter.transform_engine_v2_input_sns_arn[0].value, transform_engine_kms_key_arn = data.aws_ssm_parameter.transform_engine_v2_kms_key_arn[0].value })
...
resource "aws_lambda_event_source_mapping" "transform_engine_v2_retry_sqs_queue_mapping" {
  count            = local.transform_engine_v2_count
  event_source_arn = local.transform_engine_v2_retry_queue
  function_name    = aws_lambda_function.notifications_lambda_function.*.arn[0]
  batch_size       = 1
}
