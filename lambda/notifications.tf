...
resource "aws_kms_ciphertext" "environment_vars_notifications" {
  for_each = local.count_notifications == 0 ? {} : { slack_webhook = data.aws_ssm_parameter.slack_webhook[0].value, to_email = "tdr-secops@nationalarchives.gov.uk", muted_vulnerabilities = join(",", var.muted_scan_alerts), transform_engine_output_sqs = data.aws_ssm_parameter.transform_engine_output_sqs[0].value }
...
data "aws_ssm_parameter" "transform_engine_output_sqs" {
  count = local.count_notifications
  name  = "/${local.environment}/transform_engine/output_sqs"
}
