resource "aws_sqs_queue" "sqs_queue" {
  ...
  policy                     = templatefile("./tdr-terraform-modules/sqs/templates/${var.sqs_policy}_policy.json.tpl", { region = var.region, environment = local.environment, account_id = data.aws_caller_identity.current.account_id, sqs_name = local.sqs_name, topic_arns = local.sns_topic_arns })
